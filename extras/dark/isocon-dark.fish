# Isocon Dark theme for fish

set -g fish_color_normal 9a9a9a
set -g fish_color_command 6e9be7
set -g fish_color_keyword b586e8
set -g fish_color_quote 4cad66
set -g fish_color_redirection 49a1a6
set -g fish_color_end b586e8
set -g fish_color_error ea7a73
set -g fish_color_param 9a9a9a
set -g fish_color_comment 686868
set -g fish_color_selection --background=9a9a9a --color=282c34
set -g fish_color_search_match --background=9a9a9a --color=282c34
set -g fish_color_operator 49a1a6
set -g fish_color_escape b586e8
set -g fish_color_autosuggestion 686868
set -g fish_pager_color_progress 686868
set -g fish_pager_color_prefix 6e9be7
set -g fish_pager_color_completion 9a9a9a
set -g fish_pager_color_description 686868
